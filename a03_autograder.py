"""
IS 303 Autograder - A03: Loops and Patterns
...
Usage:
    python a03_autograder.py

    By default, the autograder looks for .py files in the current directory
    and uses a03_rubric.json for grading instructions. You can also pass
    a folder path as an argument to grade a specific folder.

    python a03_autograder.py /path/to/student/folder
...
Checks performed:
    ...
    3. Structural warnings: checks for for-loop and list/dict usage in code
    4. Output tests: runs the program with multi-item inputs and checks
       that summary output includes expected patterns (totals, max, etc.)
"""
...
def check_file_contents(file_path, problem_rubric):
    ...
    (like checking for a for loop, which earns no points but warns if missing).
    """
    points = 0
    notes = []
...
def run_test_cases(file_path, problem_rubric, timeout=15):
    ...
    A03 test cases are "output_check" type: they send multi-item inputs
    and check that the summary output contains expected patterns
    (like an average, a max, or a total).
    """
    points = 0
    notes = []
    ...
    for tc in test_cases:
        ...
        try:
                ...
                if stderr:
                    ...
                    if "EOFError" in last_line:
                        ...
                        notes.append(
                            ...
                            f"expected pattern (count then items, or "
                            f"sentinel-based)."
                        )
                        notes.append(
                            f"    The autograder sends: number of items, "
                            f"then data for each item."
                        )
                    else:
            ...
            if re.search(expected, output, re.DOTALL | re.IGNORECASE):
                ...
                notes.append(
                    f"    Note: The autograder looks for keywords like "
                    f"'total', 'average', 'highest', etc. in your output."
                )
                notes.append(
...
def main():
    ...
    rubric_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "a03_rubric.json"
    )

    if not os.path.exists(rubric_path):
        print(f"Error: Cannot find a03_rubric.json at {rubric_path}")
    ...
    print()
    print("=" * 60)
    print("  IS 303 Autograder: A03 - Loops and Patterns")
    print("=" * 60)
    ...
    print()
    print("  Note: This score does NOT include points for loop structure,")
    print("  named pattern quality, code organization, f-string formatting,")
    print("  commit messages, or GitHub submission. Those are graded by")
    print("  your instructor.")
    print()
    print("  The autograder sends inputs as: number of items, then data for")
    print("  each item. If your program uses a different input method (like")
    print("  a sentinel value), some tests may fail. Your instructor will")
    print("  test your program manually in that case.")
    print()
    print("=" * 60)
